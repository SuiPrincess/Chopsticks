...
- 毒ルールは最適応答で退化する（開始時全手が指1本＝全タップが毒相討ちになり、
  パリティだけで勝敗が決まる。手数を増やしても直らない＝参照実装の自己対戦で
  全構成3手決着を確認済み）。今日の挑戦/今週の試練の生成には毒を入れないこと。
  おまかせルールでは人間同士の遊び向けに「毒→3本手＋分割」制約つきで残している
