...
- **⚔️ 今週の試練**: 週替わりの高難度チャレンジ。難易度は常に「鬼」＋週替わりの特殊ルール構成（ISO週シードで全ユーザー共通）。全構成が「先手（人間）に勝ち筋がある」ことを厳密ソルバーで検証済み。週1回のクリア記録つき
- **📖 攻略ガイド**: キル計算・即死圏・分割・ループ・特殊ルール別のコツを図解で解説。CPUに負けた直後のリザルトからも開けて「悔しい→上達」の導線に
- **🎁 報酬テーマ**: 「今日の挑戦」通算7回クリアで「ミッドナイト」、難易度「鬼」撃破で「オニ」を解放（課金不要の実績報酬）
...
`ChopsticksTests` にゲームルール・AI・ViewModel・マルチプレイ・ストアのユニットテスト75件。
